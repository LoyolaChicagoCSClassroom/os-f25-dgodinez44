...
void list_append(struct pppage **head, struct ppage *list_to_add){
  if (*head == NULL){
    *head = list_to_add;
    return;
  }
...
void init_pfa_list(void){
  ...
  for (int i = 0; i < 128; i++){
    ...
    list_add_front(&free_physical_pages, &physical_page_array[i]);
  }
